...
export function deriveExecutiveKpis(logs: LogEntry[]): ExecutiveKpis {
  ...
  for (const log of logs) {
    ...
    if (data.isMatch === true) {
      riskLevel = "low";
      riskLabel = "Low - secure";
    }

    if (data.isMatch === false || data.riskLevel === "CRITICAL_FRAUD_ALERT") {
      riskLevel = "high";
      riskLabel = "High - escalated";
      actionLabel = "Payment blocked and escalated";
      statusLabel = "Fraud hold";
      timeSavedMinutes = 15;
    }
    ...
    if (data.action === "POST_TO_ERP_LEDGER" || log.level === "success") {
      riskLevel = "low";
      riskLabel = "Low - secure";
      actionLabel = "Auto-posted to enterprise ledger";
      statusLabel = "Approved for auto-pay";
      timeSavedMinutes = 12;
    }
