...
| Open this | Meaning |
|-----------|---------|
| `lib/migrate/runtime.ts` / `lib/workflow/runtime.ts` / `lib/privacy/runtime.ts` | `DEMO_MODE = "mockup"` |
| `lib/payflow/runtime.ts` | `DEMO_MODE = "live"` |
| `lib/migrate/engine.ts` / `lib/workflow/state-machine.ts` / `lib/privacy/engine.ts` | Mockup runtime the site executes |
| `lib/migrate/config.ts` / `lib/workflow/config.ts` / `lib/privacy/config.ts` | Prod config shape - unused in mockup |
| `lib/migrate/adapter.ts` / `lib/workflow/adapter.ts` / `lib/privacy/adapter.ts` | Chooses mockup vs live engine |
...
| **Privacy** `/privacy` | `mockup` | `lib/privacy/engine.ts` via `getPrivacyEngine()` | `lib/privacy/config.ts` (edge-middleware shape; not wired on site) |
...
See `.env.example`. Entries marked for `DEMO_MODE=live` are unused while Migrate / Workflow / Privacy stay on mockup.
